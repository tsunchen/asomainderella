        ...
        def wrapper(model):
            def inner_wrapper(self, *args, **kwargs):
                ...
                if ( orgName != "..." and orgName != "" ):
                    ...
                    # 2024-7-31 make the execute last command method
                    # print(f' - Calling {model.__name__}, arguments: {args}, {kwargs}')
                    self.last_command = model.__name__
                    self.btn_last_command.config(text = f" Execute {model.__name__} Again ", fg = "Green")
                else:
