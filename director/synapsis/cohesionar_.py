...
class Softcohesion:
    ...
    # condition = (0 == 0)
    # model = "Model"
    # filter_message = " Please select the organization about tenant firstly. "
    # filter_parameter1_ = "..."
    # filter_parameter2_ = ""
    @staticmethod
    def Intention_IFELSE_(*dargs, **dkwargs):
        """
        Intention_IFELSE_
        """
        def wrapper(model):
            def inner_wrapper(self, *args, **kwargs):
                orgName = self.pulldevicenameByTenant.get()
                if ( orgName != dkwargs['filter_parameter1_'] and orgName != dkwargs['filter_parameter2_'] ):
                    dkwargs['model'](orgName, *args, **kwargs)
                else:
                    self.file_message(dkwargs['filter_message'])
                ## print(dkwargs['condition'])
                ## print(dkwargs['model'])
            return inner_wrapper
        return wrapper
    ...
    @staticmethod
    def Intention_IFELSE__(*dargs, **dkwargs):
        """
        Intention_IFELSE__
        """
        def factory(model):
        ...
        def wrapper(model):
            def inner_wrapper(self, *args, **kwargs):
                ...
                if ( orgName not in decokwargs['condition'] ):
                    ...
                    self.file_message(decokwargs['filter_message'])
                ## print(decokwargs['condition'])
                ## print(decokwargs['model'])
            return inner_wrapper
        return wrapper
