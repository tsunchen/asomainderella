...
from tkinter import LabelFrame, Canvas, Scrollbar, VERTICAL, HORIZONTAL, RIGHT, Y, Frame, BOTTOM, X, IntVar, NSEW, LEFT, BOTH

class Pickup(ttk.Frame):
    def __init__(self, master = None, values = [], entry_idw = None,
        ...
        self.bind("<FocusIn>", lambda event: self.event_generate('<<PickupFocusIn>>'))
        self.bind("<FocusOut>", lambda event: self.event_generate('<<PickupFocusOut>>'))

        F = LabelFrame(self)
        F.pack(fill = 'x')
        self.canvas = Canvas(F, scrollregion = (0, 0, 500, (len(self._values) * 128)))
        ...
        sbar = Scrollbar(F, orient = HORIZONTAL)
        ...
        vbar.config(command = self.canvas.yview)
        sbar.config(command = self.canvas.xview)
        self.canvas.config(width = 300, height = 150)
        self.canvas.config(yscrollcommand = vbar.set, xscrollcommand = sbar.set)
