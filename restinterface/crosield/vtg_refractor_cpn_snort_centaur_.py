...
from .vtg_volumector_ import CPN


def cpninspectorModel(orgName):
    # 2022.12.13 globalless
    setattr(VTGThread_refractor, "vtg_cpn_snort_intf_", [])
    setattr(VTGThread_refractor, "vtg_cpn_snort_intf_id_", [])
    # 2021.1.20 _refractor
    setattr(VTGThread_refractor, "run", cpn_snort)
    ...
    try:
        ...
        for p in proc_bar:
            ...
            proc_bar.set_postfix({"Cloudviron": '{}'.format(p)})
            p.join()
        dfprint({'Cloudviron': VTGThread_refractor.vtg_cpn_snort_intf_, 'IntfId': VTGThread_refractor.vtg_cpn_snort_intf_id_})


def fmttab_(data):
    df = data.sort_values(by = 'Cloudviron', ascending = True)
    return df[~df.Cloudviron.isin([
        "N.P.A".center(30, ' '),
...
def dfprint(data):
    ...
    ex_centaur_ = example.groupby('IntfId',as_index = False).apply(fmttab_) # 2022.1.25 fmttab_
    print('\r{}'.format(ex_centaur_), end = '') # 2022.5.10 print\r
    print()


def cpn_snort(self):
    setattr(VTG_interface, "default_cpn_table_", default_cpn_table_)
    ...
    with VTG_interface(devicename = self.__deviceName, deviceorg = self.__deviceOrg) as (uio, intf):
        if (Wansnort['CRITICAL'].value[0] == uio):
            ...
            self.vtg_cpn_snort_intf_id_.append(self.__deviceName)
            self.vtg_cpn_snort_intf_.append(Wansnort['CRITICAL'].value[0].center(30, '.'))
        else:
            for butt in intf.orgintfcloudnet_(uio):
                if "up" == butt['if-oper-status']:

                    ## hostName = intf.default_cpn_table_
                    ## 2024.7.16 Volume Vector for CPN
                    hostName = CPN(volume = "Volume-Vector").default_vector(intf.orgName)

                    if isinstance(hostName, str):
                        # (" ~ pageJson_pings_wan : ", pageJson_pings_wan_, self.__deviceName)
                        if "success" != VTG_device(devicename = self.__deviceName).pingw_(hostname = hostName, routingInstance = butt['vrf'], count = 1):
                            self.failover(butt, intf, hostName)
                    else:
                        ...
                        for i in range(len(hostName)):                       
                            ...
                            # '''
                                ...
                                print(".T.", self.__deviceName, butt['vrf'], hostName[i])
                            # '''
                else:
...
def failover(self, butt, intf, hostname):
    ...
    try:
        ...
        # ("handler.park")
        self.park(butt, intf, Wansnort['PARK'].value[0], hostname)
    finally:
        # ("handler.overlte")
        self.overlte(butt, intf, Wansnort['OVERLTE'].value[0])


def park(self, butt, intf, tag, hostname):
    # PARK, Suspended Pause Parking [NHP]
    text_intf_brief_ = ''.join([ butt['vrf'], ' ', tag, ' ', hostname ])
    self.vtg_cpn_snort_intf_id_.append(self.__deviceName)
    self.vtg_cpn_snort_intf_.append(text_intf_brief_.center(30, '.'))
