...
class VTG_remprintal:
    ...
    __slots__ = (
        '__help', '__mb_html_b', '__mb_html_c', 
        '__event_of_alarms_', '__number_of_alarms_', '__detail_of_alarms_'
    ...
    async def mAil_printal_(self, contents, recvs, ccrecvs, subjectas):
        host = self.__class__.host
        sender = self.__class__.sender
        receivers = recvs
        subject = 'Hey! Mail Monitor Printer {} Powered by TSunx'.format(subjectas)
        ccRecvs = ccrecvs
        try:
            server = smtplib.SMTP()
            server.connect(host)
